...
class PairTradingStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        for a, b in self.combs:
            ...
            self.transform = btind.OLS_TransformationN(self.data0.adjclose, self.data1.adjclose,
            ...
            self.beta =  btind.OLS_BetaN(self.data0.adjclose, self.data1.adjclose,
                                                       period=self.p.period,plot = False)

            # self.spread = spread_class(self.data0, self.data1, period=self.p.period)

            self.coint = btind.CointN(self.data0.adjclose, self.data1.adjclose,
                                        period=self.p.period,plot = False)
    ...
    def next(self):
        ...
        if len(self) > self.p.period:
            ...
            for i in self.combs_stats:
                ...
                self.spread = self.data0.adjclose[0] -(self.beta[0]*self.data1.adjclose[0])
                ...
                #         self.plts[self.sp][1]['close'][0].append(self.data0.datetime.datetime().strftime('%Y%m%d'))
                ...
                if (self.zscore[0] > self.upper_limit) and self.zscore[0] < self.stop_up and self.pos_traker[self.sp] != 1 and allowed_new_pos == True: #and self.transform.spread < self.transform.spread_twostd:#(self.status != 1):
                ...
                elif (self.zscore[0] < self.lower_limit) and self.zscore[0] > self.stop_down and self.pos_traker[self.sp] != 2 and allowed_new_pos == True: # and self.transform.spread > self.transform.spread_negtwostd:
                ...
                elif self.pos_traker[self.sp]!=0 and (self.zscore[0] < self.stop_down or self.zscore[0] > self.stop_up):
