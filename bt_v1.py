...
#
# Implement Coint test from ols indicator
###
...
class PairTradingStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if (self.zscore[0] > self.upper_limit) and (self.status != 1):
            ...
            y = size * hr  # Find the number of shares for Stock2


            # Placing the order
            self.log('SELL CREATE %s, price = %.2f, qty = %d' % ("HST", self.data0.close[0], x))
            self.sell(data=self.data0, size=(x + self.qty1))  # Place an order for buying y + qty2 shares
            self.log('BUY CREATE %s, price = %.2f, qty = %d' % ("VNO", self.data1.close[0], y))
            ...
            # Place the order
            self.log('BUY CREATE %s, price = %.2f, qty = %d' % ("HST", self.data0.close[0], x))
            self.buy(data=self.data0, size=(x + self.qty1))  # Place an order for buying x + qty1 shares
            self.log('SELL CREATE %s, price = %.2f, qty = %d' % ("VNO", self.data1.close[0], y))
            ...
            self.log('CLOSE POS %s, price = %.2f' % ("HST", self.data0.close[0]))
            self.close(self.data0)
            self.log('CLOSE POS %s, price = %.2f' % ("VNO", self.data1.close[0]))
