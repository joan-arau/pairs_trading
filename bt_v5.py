...
class PairTradingStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        for a, b in self.combs:
            ...
            self.plts[str(a+'/'+b)] = [[],{'long':[[],[]],'short':[[],[]],'close':[[],[]],'stop':[[],[]],'coint':[[],[]]}]
    ...
    def next(self):
        ...
        if len(self) > self.p.period:
            ...
            for i in self.combs_stats:
                ...
                if self.coint.pvalue[0] > self.p.coint_pvalue_cutoff:
                    ...
                    if self.spread_traker[self.sp][0] != 0:
                        ...
                        orders['close'].append(
                            {'spread': self.a + '/' + self.b, self.a: -self.spread_traker[self.sp][2] , self.b: -self.spread_traker[self.sp][3],
                             'tracker': [0,0,0,0],'kind':'coint','z':self.zscore[0],'date':self.data0.datetime.datetime()})
                        #
                        # orders['close'][self.a] = self.pos_traker[self.a] - self.getposition(data=self.data_dic[self.a]).size
                        # orders['close'][self.b] = self.pos_traker[self.b] - self.getposition(data=self.data_dic[self.b]).size
                    ...
                    # if (self.transform.spread[0] > self.transform.spread_std[0]) and self.pos_traker[self.sp] != 1:#(self.status != 1):
                        ...
                        orders['open'].append({'spread': self.a+'/'+self.b,'z':self.zscore[0],self.a:-x,self.b:y,'tracker':[1,size,-x,y],'kind':'short','date':self.data0.datetime.datetime()})
                        orders['open'][-1]['$'] = (orders['open'][-1][self.a] * self.data0.close[0]) + (orders['open'][-1][self.b] * self.data1.close[0])
                    ...
                    # elif (self.transform.spread[0] < self.transform.spread_twostd[0]) and self.pos_traker[self.sp] != 2:
                        ...
                        orders['open'].append({'spread': self.a + '/' + self.b, 'z': self.zscore[0],
                                               self.a: x,
                                               self.b: -y,
                                               'tracker':[2,size,x,-y],'kind':'long','date':self.data0.datetime.datetime()}
                                              )
                        orders['open'][-1]['$'] = (orders['open'][-1][self.a]* self.data0.close[0]) +(orders['open'][-1][self.b]* self.data1.close[0])
                    ...
                    # elif (self.transform.spread > 0.1 *self.transform.spread_negOnestd and self.transform.spread < 0.1*self.transform.spread_std):
                        ...
                        orders['close'].append(
                            {'spread': self.a + '/' + self.b,
                             self.a: -self.spread_traker[self.sp][2],
                             self.b: -self.spread_traker[self.sp][3],
                             'tracker': [0, 0, 0, 0],'kind':'close','z':self.zscore[0],'date':self.data0.datetime.datetime()})
                    ...
                    # elif (self.transform.spread < self.transform.spread_negtwostd and self.pos_traker[self.sp] != 0) or( self.transform.spread > self.transform.spread_twostd  and self.pos_traker[self.sp] != 0):
                        ...
                        orders['close'].append(
                            {'spread': self.a + '/' + self.b,
                             self.a: -self.spread_traker[self.sp][2],
                             self.b: -self.spread_traker[self.sp][3],
                             'tracker': [0, 0, 0, 0],'kind':'stop','z':self.zscore[0],'date':self.data0.datetime.datetime()})


            # print(self.pos_traker)
            # print(self.spread_traker)

            # print('closing orders: ',pd.DataFrame(orders['close'], index = [0]))
            df = pd.DataFrame(orders['close'])
            if len(orders['close']) == 0 and len(orders['open']) != 0 :
                # print('opening orders: ',pd.DataFrame(orders['open']).sort_values(by = ['z'],ascending = False))

                df = pd.DataFrame(orders['open'])
                df = df.iloc[(-df['z'].abs()).argsort()].reset_index(drop=True).iloc[[0]]
                # df = pd.DataFrame(orders['open']).sort_values(by = ['z'],ascending = False).iloc[[0]]
                # df = pd.concat([df,df2]).dropna(how = 'all')
                # print('df',df)
            # print(df)
            ...
            if len(df.index) >= 1:
                for index,i in df.dropna(how = 'all').iterrows():
                    # print(i)
                    # print(i['spread'],i['tracker'])
                    self.spread_traker[i['spread']] = i['tracker']
                    self.plts[i['spread']][1][i['kind']][0].append(i['date'])
                    self.plts[i['spread']][1][i['kind']][1].append(i['z'])

            # print(self.spread_traker)

            d = {}
            cols = set(list(df.columns)).difference({0,'spread','z','$','tracker','kind','date'})
            # print('cols ', cols,list(df.columns),{'spread','z','$'})
            for i in cols:
            ...
            if len(d.keys()) > 0:
                for i in d.keys():
                    ...
                    self.pos_traker[i] = self.getposition(data=self.data_dic[i]).size

            for i in list(self.data_dic.keys()):

                delta_beta = self.getposition(data=self.data_dic[i]).size * self.bench_beta[i][0]
                # print('s', self.getposition(data=self.data_dic[i]).size,' b ',self.bench_beta[i][0])

                print(i, ' POS: ',self.getposition(data=self.data_dic[i]).size, ' BETA: ', delta_beta)
    ...
    def stop(self):
        ...
        if self.p.plot:
            for i in self.plts.keys():
                ...
                axs[2].scatter(self.plts[i][1]['coint'][0], self.plts[i][1]['coint'][1], 500, color='yellow', marker='s')
...
def runstrategy(ticker_list,bench_ticker):
    ...
    dic = {'SQN': printSQN(strat[0].analyzers.sqn.get_analysis()),
           ...
           'max_dd': round(empyrical.max_drawdown(return_df)[0] * 100,2),
           'max_dd_bm':round(empyrical.max_drawdown(bench_df)[0] * 100,2),
           'beta': beta,
