...
class PairTradingStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if (self.zscore[0] > self.upper_limit) and (self.status != 1):
            ...
            # Placing the order
            self.log('SELL CREATE %s, price = %.2f, qty = %d' % ("AAPL", self.data0.close[0], x + self.qty1))
            self.sell(data=self.data0, size=(x + self.qty1))  # Place an order for buying y + qty2 shares
            self.log('BUY CREATE %s, price = %.2f, qty = %d' % ("MSFT", self.data1.close[0], y + self.qty2))
            ...
            # Place the order
            self.log('BUY CREATE %s, price = %.2f, qty = %d' % ("HST", self.data0.close[0], x + self.qty1))
            self.buy(data=self.data0, size=(x + self.qty1))  # Place an order for buying x + qty1 shares
            self.log('SELL CREATE %s, price = %.2f, qty = %d' % ("VNO", self.data1.close[0], y + self.qty2))
